  ...
  switch (field.name) {
    ...
    case "ingredients":
      if (!/^.{3,255}$/.test(field.value)) {
        ...
        if (field.value.length < 3) {
          setHelperText({ ...helperText, ingredients: "Es muy corto" });
        } else if (field.value.length > 255) {
          setHelperText({ ...helperText, ingredients: "Es muy largo" });
        } else {
    ...
    case "size":
      if (!/^[a-zA-Z0-9]{2,25}$/.test(field.value)) {
        ...
        if (field.value.length < 2) {
          setHelperText({ ...helperText, size: "Es muy corto" });
        } else if (field.value.length > 25) {
          setHelperText({ ...helperText, size: "Es muy largo" });
        } else {
          setHelperText({ ...helperText, size: "No puede tener espacios" });
        }
      } else {
    ...
    case "brand":
      if (!/^.{2,25}$/.test(field.value)) {
        ...
        if (field.value.length < 2) {
          setHelperText({ ...helperText, brand: "Es muy corto" });
        } else if (field.value.length > 25) {
          setHelperText({ ...helperText, brand: "Es muy largo" });
        } else {
    ...
    case "price":
      if (!/^[0-9,.]{2,9}$/.test(field.value)) {
        ...
        if (field.value.length < 2) {
          setHelperText({ ...helperText, price: "Es muy corto" });
        } else if (field.value.length > 9) {
          setHelperText({ ...helperText, price: "Es muy largo" });
        } else {
          setHelperText({
            ...helperText,
            price: "No permite caracteres que no sean números",
          });
        }
      } else {
    ...
    case "sku":
      if (!/^[0-9a-zA-Z]{2,12}$/.test(field.value)) {
        ...
        if (field.value.length < 2) {
          setHelperText({ ...helperText, sku: "Es muy corto" });
        } else if (field.value.length > 9) {
          setHelperText({ ...helperText, sku: "Es muy largo" });
        } else {
